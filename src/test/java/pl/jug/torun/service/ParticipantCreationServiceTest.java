...
public class ParticipantCreationServiceTest extends AbstractTransactionalJUnit4SpringContextTests {
    ...
    @Test
    public void shouldGetExistingUserFromRepository() {
        List<Participant> downloadedParticipants = Arrays.asList(existing);

        List<Participant> results = participantCreationService.createParticipantsIfNotExists(downloadedParticipants);

        assertEquals(1, results.size());
        assertEquals(existing.getId(), results.get(0).getId());
    }
